...
```bash
# 세션 목록 조회
python app/main.py --session list

# 특정 세션 정보
python app/main.py --session info --session-id <session_id>

# 대화 내역 조회
python app/main.py --session history --session-id <session_id>

# 세션 삭제
python app/main.py --session delete --session-id <session_id>

# 모든 세션 삭제
python app/main.py --session clear
```

### 세션 전략 설정
```bash
# 터미널별 세션 (기본값)
python app/main.py -c "안녕하세요" --session-strategy terminal_pid

# 사용자별 세션
python app/main.py -c "안녕하세요" --session-strategy system_user

# 커스텀 세션
python app/main.py -c "안녕하세요" --session-strategy custom --user-id "my_session"

# 디렉토리별 세션
python app/main.py -c "안녕하세요" --session-strategy directory

# 최근 세션 사용
python app/main.py -c "안녕하세요" --session-strategy recent
```
...
```bash
# 일정 생성
$ python app/main.py -c "내일 오후 3시에 회의 일정 추가해줘"
🤖 내일 오후 3시에 '회의' 일정을 추가했습니다.

# 일정 조회
$ python app/main.py -c "이번 주 일정 알려줘"
🤖 이번 주 일정을 확인해드리겠습니다...

# 일정 수정
$ python app/main.py -c "회의 시간을 4시로 변경해줘"
...
```bash
# 첫 번째 대화
$ python app/main.py -c "안녕하세요! 저는 김덕배입니다."
🤖 안녕하세요, 김덕배님! 만나서 반갑습니다.

# 두 번째 대화 (이름 기억)
$ python app/main.py -c "내가 누구라고?"
